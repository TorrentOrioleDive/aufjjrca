...
3. Выполни:
   ```js
   localStorage.getItem("audio-chat-uid")
   ```
   — это твой `token`.
4. Запиши `navigator.userAgent` (это твой `ua`) — он будет использован при подключении из бота.
5. Полезно перед извлечением токена немного пообщаться (1–2 успешных звонка), чтобы антибот-флаги не сработали при первом же подключении бота.
